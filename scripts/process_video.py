...
def extract_frames_fixed(
    video_path, output_dir, num_frames, downscale_factor, start_number=0, video_idx=0
):
...
def extract_precise_geometry(
    ...
    extractor = (
        SuperPoint(max_num_keypoints=1024, detection_threshold=0.01).eval().to(device)
    )
    ...
    prev_corners = np.float32(
        [[0, 0], [0, h_proc], [w_proc, h_proc], [w_proc, 0]]
    ).reshape(-1, 1, 2)
                ...
                if valid.sum() > 10:
                    ...
                    if use_yolo:
                        ...
                        if (
                            ...
                            combined_mask = (
                                masks[largest_idx].cpu().numpy().astype(bool)
                            )
                            if combined_mask.shape[:2] != (h_proc, w_proc):
                                combined_mask = cv2.resize(
                                    combined_mask.astype(np.uint8), (w_proc, h_proc)
                                ).astype(bool)
                    ...
                    if M is not None:
                        curr_corners = np.float32(
                            [[0, 0], [0, h_proc], [w_proc, h_proc], [w_proc, 0]]
                        ).reshape(-1, 1, 2)
                        projected_corners = cv2.perspectiveTransform(curr_corners, M)
                        try:
                            ret_area, intersection_contour = cv2.intersectConvexConvex(
                                prev_corners, projected_corners
                            )
            ...
            if should_save:
                if downscale_factor > 1:
                    ...
                    save_frame = cv2.resize(
                        frame_full, (w // downscale_factor, h // downscale_factor)
                    )
                else:
            ...
            if show_gui:
                ...
                if matched_kpts_curr is not None:
                    for kp in matched_kpts_curr:
                        cv2.circle(
                            vis_img, (int(kp[0]), int(kp[1])), 3, (0, 255, 0), -1
                        )
                ...
                cv2.putText(
                    vis_img,
                    f"Overlap: {calculated_overlap:.2f}",
                    (10, 30),
                ...
                proc_fps = (
                    stats["processed_frames"] / elapsed_proc if elapsed_proc > 0 else 0
                )
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_false",
        dest="adaptive",
        help="Disable adaptive mode",
    )
    parser.add_argument(
        "--no-yolo", action="store_false", dest="yolo", help="Disable YOLO segmentation"
    )
    ...
    if not video_paths:
        print(
            f"Error: Could not open video {raw_video_paths[0] if raw_video_paths else ''}"
        )
        sys.exit(1)
    ...
    if len(video_paths) > 1 and (start_time is not None or end_time is not None):
        print(
            "⚠️ Warning: Truncation (--start/--end) is only supported for single video processing. Ignoring."
        )
        start_time = None
        end_time = None
    ...
    for video_idx, video in enumerate(video_paths):
        ...
        if args.adaptive:
            ...
            count, v_stats = extract_precise_geometry(
                ...
                args.overlap,
                args.downscale,
                start_number=global_frame_count,
                video_idx=video_idx,
                use_yolo=args.yolo,
                show_gui=args.gui,
            ...
            count, v_stats = extract_frames_fixed(
                video,
                output_dir,
                args.num_frames,
                args.downscale,
        ...
        if v_stats.get("valid_car_detections", 0) > 0:
            avg_matches = (
                v_stats["total_matches_on_car"] / v_stats["valid_car_detections"]
            )
    ...
    for s in all_stats_summary:
        print(
            f"{s['name']:<25} | {s['extracted']:<6} | {s['fallback']:<10} | {s['skipped_overlap']:<7}"
        )
